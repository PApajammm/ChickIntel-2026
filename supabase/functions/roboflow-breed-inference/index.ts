...
function collectPredictions(value: unknown, predictions: BreedPrediction[]) {
...
function sortUniquePredictions(predictions: BreedPrediction[]) {
  const byClassName = new Map<string, BreedPrediction>();
...
Deno.serve(async (request) => {
  try {
    ...
    const roboflowModelId =
      Deno.env.get("ROBOFLOW_BREED_MODEL_ID") ?? CHICKEN_BREED_MODEL_ID;
