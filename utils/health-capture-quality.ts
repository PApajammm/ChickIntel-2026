...
export function buildHealthCaptureGuidance(
  ...
  const guidance = assessment.issues.map((issue) => {
    switch (issue) {
      ...
      case "low_resolution":
        return "Move closer or hold the phone steady so the chicken stays sharp and large enough in frame.";
      case "too_wide":
        return "Keep the chicken more centered in the guide and avoid capturing too much empty background.";
      case "too_tall":
