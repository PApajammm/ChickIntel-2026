...
function decodeScheduleCategoryValue(
  rawCategory: string,
): EncodedScheduleCategory {
  ...
  try {
    ...
    return {
      ...
      feedDailyAmount:
        parsed.feedDailyAmount === null || parsed.feedDailyAmount === undefined
          ? null
...
export function scheduleTaskMatchesDate(
  task: Pick<
    ...
    "startDate" | "endDate" | "repeat" | "customRepeatDays"
  >,
  date: Date,
) {
  ...
  switch (task.repeat) {
    ...
    case "Custom":
      return (
        task.customRepeatDays?.includes(SCHEDULE_DAYS_OF_WEEK[date.getDay()]) ??
        false
      );
    default:
      return true;
...
export function countScheduleTaskOccurrencesThroughDateTime(
  ...
  if (
    endDateTime.getTime() <
    buildOccurrenceDateTime(startDate, task.time).getTime()
  ) {
    return 0;
  }
...
export function computeTaskStatus(
  ...
  const graceWindowEnd = new Date(
    scheduledTimeToday.getTime() + 30 * 60 * 1000,
  );
...
export async function completeScheduleTask(
  ...
  if (error) {
    // Fallback: check if the completion record was already inserted
